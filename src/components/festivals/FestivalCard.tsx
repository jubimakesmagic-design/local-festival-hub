  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-lg border border-border bg-card shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md">
      {hasRealPoster ? (
        ...
        <div className={`relative h-16 w-full border-b border-border/40 bg-gradient-to-r ${gradientClass} flex items-center justify-between px-3.5 sm:px-4 shrink-0`}>
          <FestivalStatusBadge startDate={festival.startDate} endDate={festival.endDate} />
          {festival.trustScore >= 80 && (
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-1.5 text-base font-extrabold text-muted-foreground sm:gap-2">
          ...
          <Link href={`/festivals/${festival.id}`} className="block">
            <h2 className="mb-3 line-clamp-2 text-xl font-extrabold leading-snug text-foreground transition-colors group-hover:text-primary sm:text-2xl">
              {festival.name}
            </h2>
          </Link>

          <p className="mb-4 line-clamp-3 text-base leading-7 text-muted-foreground">
            {festival.description || "축제 소개가 아직 보강 중입니다. 공식 출처가 확인되는 대로 업데이트됩니다."}
          </p>
        </div>

        <div className="mb-4 flex min-h-10 flex-wrap gap-2 border-y border-border py-3 text-muted-foreground">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 text-base font-extrabold text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:gap-3">
        ...
        <div className="grid grid-cols-[1fr_auto] gap-2 pt-4">
          <Link href={`/festivals/${festival.id}`} className="block">
            <button className="h-14 w-full cursor-pointer rounded-lg text-lg font-extrabold press-button">
              자세히 보기
            </button>
          </Link>
          {festival.officialUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex h-14 w-14 items-center justify-center rounded-lg border border-border bg-background text-muted-foreground transition hover:border-primary hover:text-primary"
              title="공식 사이트 열기"
            >
...
function FeaturePill({
  ...
  return (
    <div className="flex min-h-9 items-center gap-1 rounded-full bg-secondary px-3 py-1.5 text-sm font-extrabold" title={title}>
